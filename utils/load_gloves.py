...
class model_glove:
    ...
    def __init__(self, config_glove, corpus,col='Token'):
        ...
        #load training data
        self.corpus = corpus
        self.embedding_maxsize_corpus=max(corpus[col])
    ...
    def text_emb(self,vect,already_tokkenized=True):
        ...
        if already_tokkenized ==False :            
            token_vect = self.text_preprovess(vect)
    ...
    def text_preprocess(self,raw_text):
        ...
        text=self.counter.transform([text])
    ...
    def cosine_sim_text(self,vect):
        ...
        vect = self.text_emb(vect, already_tokkenized=False)
    ...
    def n_answer(self,text,n=10):
        '''
        n closest answer to the corpus
        '''
        n_first=pd.DataFrame(columns=self.corpus.columns)
    
        liste_answer = self.cosine_sim_text(text)
    
        for i in range (0,n):
            key=liste_answer[i][0]
            n_first=n_first.append(self.corpus.iloc[key], ignore_index = True)
